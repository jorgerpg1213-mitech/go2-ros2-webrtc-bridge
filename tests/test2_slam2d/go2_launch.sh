...
echo "[slam] instalando slam_toolbox (una vez)..."
docker exec go2_ros bash -c "apt-get update -qq && apt-get install -y -qq ros-humble-slam-toolbox > /dev/null 2>&1" && echo "[slam] listo"
...
sleep 1
dexec_bg "slam" "ros2 run slam_toolbox async_slam_toolbox_node --ros-args --params-file /scripts/slam_params.yaml"
sleep 1
