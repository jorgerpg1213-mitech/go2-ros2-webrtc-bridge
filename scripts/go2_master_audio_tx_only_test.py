...
async def _audio_handler(frame):
    global _audio_recv, _audio_out, _audio_rx_rms_acc, _audio_rx_rms_n, _audio_rx_peak
    _audio_recv += 1
    try:
        arr = frame.to_ndarray()
        data = arr.reshape(frame.samples, 2).astype(np.int16, copy=False)
        d32 = data.astype(np.float32)
        _audio_rx_rms_acc += float(np.sqrt(np.mean(d32 * d32)))
        _audio_rx_rms_n += 1
        pk = int(np.max(np.abs(data))) if data.size else 0
        ...
        if _audio_out is None:
            ...
            _audio_out.start()
            log("AUDIO_IN", "speaker abierto default sr=48000 ch=2")

        _audio_out.write(data)

        if _audio_recv <= 5:
            log("AUDIO_IN", f"play frame#{_audio_recv} data={data.shape} min={data.min()} max={data.max()}")
    except Exception as e:
...
async def status_loop() -> None:
    ...
    while state.connection_state != LifecycleState.SHUTDOWN:
        ...
        try:
            ...
            log("AUDIO_RX", f"frames={_audio_recv} rms={rx_rms:.1f} peak={_audio_rx_peak}")
            _audio_rx_rms_acc = 0.0
            _audio_rx_rms_n = 0
            _audio_rx_peak = 0
        except Exception as e:
