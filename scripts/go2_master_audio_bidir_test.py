...
import unitree_webrtc_connect.webrtc_audio as _go2_audio
...
ENABLE_AUDIO_IN = os.environ.get("ENABLE_AUDIO_IN", "1") != "0"
_audio_recv = 0
_audio_out = None
_audio_rx_rms_acc = 0.0
_audio_rx_rms_n = 0
_audio_rx_peak = 0

# Audio RX processing: robot -> PC
AUDIO_RX_GAIN = float(os.environ.get("AUDIO_RX_GAIN", "0.25"))
AUDIO_RX_HPF_HZ = float(os.environ.get("AUDIO_RX_HPF_HZ", "120.0"))
AUDIO_RX_LIMIT = float(os.environ.get("AUDIO_RX_LIMIT", "0.85"))
AUDIO_RX_QUEUE_MAX = int(os.environ.get("AUDIO_RX_QUEUE_MAX", "30"))

_audio_play_q = queue.Queue(maxsize=AUDIO_RX_QUEUE_MAX)
_audio_play_thread = None
_audio_play_stop = threading.Event()
_audio_rx_hpf_x_prev = None
_audio_rx_hpf_y_prev = None
_audio_rx_play_drop = 0

# Raw RX capture: robot mic -> WAV, before any filtering/playback
AUDIO_RX_CAPTURE = os.environ.get("AUDIO_RX_CAPTURE", "1") != "0"
AUDIO_RX_PLAYBACK = True
_audio_rx_wav = None
_audio_rx_wav_path = None
_audio_rx_capture_frames = 0
...
# ─── Audio IN: robot mic -> PC speaker ───────────────────────────────────────

def _audio_rx_capture_write(data_raw: np.ndarray) -> None:
    """Write raw RTP RX audio to WAV before filters/playback."""
    global _audio_rx_wav, _audio_rx_wav_path, _audio_rx_capture_frames
    if not AUDIO_RX_CAPTURE:
        return

    if _audio_rx_wav is None:
        os.makedirs("runs/audio_captures", exist_ok=True)
        ts = time.strftime("%Y%m%d_%H%M%S")
        _audio_rx_wav_path = f"runs/audio_captures/robot_rx_raw_{ts}.wav"
        _audio_rx_wav = wave.open(_audio_rx_wav_path, "wb")
        _audio_rx_wav.setnchannels(2)
        _audio_rx_wav.setsampwidth(2)
        _audio_rx_wav.setframerate(48000)
        log("AUDIO_CAPTURE", f"raw WAV abierto path={_audio_rx_wav_path}")

    _audio_rx_wav.writeframes(data_raw.astype(np.int16, copy=False).tobytes())
    _audio_rx_capture_frames += data_raw.shape[0]


# ─── DSP RX: robot mic -> PC speaker ──────────────────────────────────────────

AUDIO_RX_POST_GAIN = float(os.environ.get("AUDIO_RX_POST_GAIN", "4.0"))
AUDIO_RX_LIMIT = float(os.environ.get("AUDIO_RX_LIMIT", "0.78"))

_audio_rx_dsp_state = {}

def _biquad_coeff(kind: str, f0: float, q: float, fs: float = 48000.0):
...
_DSP_SECTIONS = [
    ("notch105", _biquad_coeff("notch",    105.0, 18.0)),
    ("notch217", _biquad_coeff("notch",    217.0, 10.0)),
    ("hpf280a",  _biquad_coeff("highpass", 280.0, 0.707)),
    ("hpf280b",  _biquad_coeff("highpass", 280.0, 0.707)),
    ("lpf3800",  _biquad_coeff("lowpass", 3800.0, 0.707)),
]

def _biquad_apply_stereo(x: np.ndarray, name: str, coeff: np.ndarray) -> np.ndarray:
    st = _audio_rx_dsp_state.get(name)
    if st is None:
        st = {
...
def _audio_rx_process(data: np.ndarray) -> np.ndarray:
    """DSP desactivado para esta corrida: passthrough crudo (sin loop biquad)."""
    ...
    # Cascada SOS estable.
    for name, coeff in _DSP_SECTIONS:
        x = _biquad_apply_stereo(x, name, coeff)

    # Subir voz después de limpiar ruido mecánico.
    x *= AUDIO_RX_POST_GAIN

    # Soft limiter final.
    limit = max(0.10, min(AUDIO_RX_LIMIT, 1.0)) * 32767.0
    x = limit * np.tanh(x / limit)

    return np.clip(x, -32768, 32767).astype(np.int16)


def _audio_rx_playback_loop():
    global _audio_out
    try:
        _audio_out = sd.OutputStream(
            samplerate=48000,
            channels=2,
            dtype="int16",
            device=None,
        ...
        _audio_out.start()
        log("AUDIO_IN", "playback thread activo sr=48000 ch=2")

        while not _audio_play_stop.is_set():
            try:
                data = _audio_play_q.get(timeout=0.1)
            except queue.Empty:
                continue
            _audio_out.write(data)

    except Exception as e:
        log_error_rate_limited("audio_playback_thread", str(e))


def _audio_rx_shutdown():
    _audio_play_stop.set()
    try:
        if _audio_play_thread and _audio_play_thread.is_alive():
            _audio_play_thread.join(timeout=1.0)
    except Exception:
        pass
    try:
        if _audio_out:
            _audio_out.stop()
            _audio_out.close()
            log("SHUTDOWN", "Audio RX cerrado")
    except Exception:
        pass
    try:
        if _audio_rx_wav:
            _audio_rx_wav.close()
            log("SHUTDOWN", f"Audio raw WAV cerrado path={_audio_rx_wav_path} frames={_audio_rx_capture_frames}")
    except Exception:
        pass


async def _audio_handler(frame):
    global _audio_recv, _audio_rx_rms_acc, _audio_rx_rms_n, _audio_rx_peak
    global _audio_play_thread, _audio_rx_play_drop

    _audio_recv += 1
    try:
        arr = frame.to_ndarray()
        data_raw = arr.reshape(frame.samples, 2).astype(np.int16, copy=False)
        _audio_rx_capture_write(data_raw)

        d32 = data_raw.astype(np.float32)
        _audio_rx_rms_acc += float(np.sqrt(np.mean(d32 * d32)))
        _audio_rx_rms_n += 1
        pk = int(np.max(np.abs(data_raw))) if data_raw.size else 0
        if pk > _audio_rx_peak:
            _audio_rx_peak = pk

        if AUDIO_RX_PLAYBACK:
            data = _audio_rx_process(data_raw)

            if _audio_play_thread is None:
                _audio_play_thread = threading.Thread(
                    target=_audio_rx_playback_loop,
                    name="audio_rx_playback",
                    daemon=True,
                )
                _audio_play_thread.start()

            if _audio_play_q.full():
                try:
                    _audio_play_q.get_nowait()
                    _audio_rx_play_drop += 1
                except queue.Empty:
                    pass
            try:
                _audio_play_q.put_nowait(data)
            except queue.Full:
                _audio_rx_play_drop += 1

            if _audio_recv <= 5:
                log("AUDIO_IN", f"processed frame#{_audio_recv} raw_peak={pk} out_min={data.min()} out_max={data.max()}")
        elif _audio_recv <= 5:
            log("AUDIO_IN", f"captured raw frame#{_audio_recv} shape={data_raw.shape} raw_peak={pk}")

    except Exception as e:
        log_error_rate_limited("audio_play", str(e))
...
async def status_loop() -> None:
    ...
    while state.connection_state != LifecycleState.SHUTDOWN:
        ...
        _prof_report()
        try:
            tx = _go2_audio.get_audio_tx_stats(reset=True)
            tx_rms = tx["rms_acc"] / tx["rms_n"] if tx["rms_n"] else 0.0
            log("AUDIO_TX", f"cb={tx['cb']} sent={tx['sent']} drop={tx['drop']} qmax={tx['qmax']} rms={tx_rms:.1f} peak={tx['peak']}")
            global _audio_rx_rms_acc, _audio_rx_rms_n, _audio_rx_peak
            rx_rms = _audio_rx_rms_acc / _audio_rx_rms_n if _audio_rx_rms_n else 0.0
            log("AUDIO_RX", f"frames={_audio_recv} rms={rx_rms:.1f} peak={_audio_rx_peak} play_drop={_audio_rx_play_drop} q={_audio_play_q.qsize()} gain={AUDIO_RX_GAIN} hpf={AUDIO_RX_HPF_HZ}")
            _audio_rx_rms_acc = 0.0
            _audio_rx_rms_n = 0
            _audio_rx_peak = 0
        except Exception as e:
            log_error_rate_limited("audio_stats", str(e))
    ...
    while not _shutdown_event.is_set():
        ...
        if ENABLE_AUDIO_IN:
            try:
                ...
                conn.audio.add_track_callback(_audio_handler)
                log("AUDIO_IN", "robot mic playback activo")
            except Exception as e:
                log_error_rate_limited("audio_in", str(e))
